...
Core tests take longer than any other FlakeChain tests, due to the high amount of computational work involved in validating core components.
...
To run only FlakeChain's core tests (after building):
...
To run only FlakeChain's crypto tests (after building):
...
```
flakechaind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `flakechain-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only FlakeChain's hash tests (after building):
...
To run only FlakeChain's performance tests (after building):
...
To run only FlakeChain's unit tests (after building):
